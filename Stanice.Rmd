...
# input <- c()
# input$entry.id <- "Bulhary3"
# input$entry.var.updt <- "SRA1H"
# input$entry.year <- c(1988)
# 
...
# data za posledni rok (od ledna 2018 do 23.09.2018 11:30)
# stanice_updt <- readRDS(file.path(.datadir, "libor_d_new/stanice_updt.rds")) 
stanice_updt <- readRDS(file.path(.datadir, "libor_d_new/stanice_updt_201809231130.rds")) 
...
for (i in list_of_files){
  ...
  dta_actual_orig = rbind(dta_actual_orig, data.frame(name = i, dta))
}

dta_actual_orig$DTM <- as.POSIXct(dta_actual_orig$DTM, tz = "GMT", format = "%Y%m%d%H%M")

zkr <- read.table(file.path(.datadir, "zkr_seznam.txt"), header = TRUE, sep = " ")
dta_actual_orig <- merge(dta_actual_orig, zkr, by = "SIGNAL_KEY")
...
st_ <- data.frame(ID = c("VUV meteo", "Kojetice potok", "Racinka", "Oracov", "Lisany",
                  ...
                  new = c("VUV meteo", "Kojeticky potok", "Racinka", "Oracov", "Lisany",
                          "Merbolticky potok", "Svitavka", "Teplice", "Bastynsky potok",
                          "Podhora", "Bulhary1", "Bulhary2", "Bulhary3"))

dta_actual_orig <- dta_actual_orig %>% left_join(st_, by="ID") %>% filter(!ID %in% c("VURV Oldrichov", "Prepere", "Bastynsky potok")) %>%
                             select(-name, -ID, ID = new)
rm(dta, st_, zkr)

choices.id <- sort(unique(as.character(poloha$Oznaceni)), decreasing = TRUE)
...
dta <- reactive({
  ...
  dta_actual <- dta_actual_orig %>% filter(ID == input$entry.id)
...
renderPlotly({
  ...
  dta.yt <- dta %>% filter(year %in% input$entry.year) %>% 
                  group_by(year) %>% mutate(month = seq(1,12,11/(n()-1))) %>% ungroup()
...
dta.median <- dta.long %>% filter(Q=="median") 
dta.long.q <- dta.long %>% filter(Q!="median") %>% group_by(Q,month) %>% summarise(Qmin = min(value), Qmax = max(value)) %>% ungroup()

# my_grey <- grey.colors(4, start = 0.3, end = 0.9, gamma = 2.2, alpha = NULL)

dta.long.q$Q <- factor(dta.long.q$Q, levels=c("Q80","Q60", "Q40", "Q20"))

g1 <-
  ggplot()+
  geom_ribbon(data = dta.long.q, aes(month, ymin=Qmin, ymax=Qmax, group=Q, fill=Q,
              text=sprintf("mesic: %s<br>kvantil: %s<br>rozpeti: %s - %s", 
                           dta.long.q$month, dta.long.q$Q, round(dta.long.q$Qmin,2), round(dta.long.q$Qmax,2))), alpha=0.25)+
  geom_line(data = dta.median, aes(month,value, color = factor(Q), group = factor(Q),
                                   text=sprintf("mesic: %s<br>median: %s", dta.median$month, round(dta.median$value,2))), alpha=0.75)+
...
renderDygraph({
  ...
  dta_actual_48 <- dta_actual %>% filter(between(DTM, now("GMT") - days(2), now("GMT")))
  ...
  RangeMin <- min(dta$value)-0.5
  RangeMax <- max(dta$value)+0.5
  
  dta <- xts::xts(dta, order.by = dta$DTM)
  
  dygraph(dta, xlab = "Čas") %>%  
  dyOptions(colors = RColorBrewer::brewer.pal(3, "Set1"), connectSeparatedPoints = TRUE) %>%
...
renderDygraph({
  ...
  actual_48 <- dta_actual %>% filter(between(DTM, now("GMT") - days(2), now("GMT")))
  
  if(nrow(actual_48) < 3){
    actual_48 <- data.frame(DTM = seq(now("GMT") - days(2), now("GMT"), by = "hours"), value = 0)
    actual_48 <- xts::xts(actual_48, order.by = actual_48$DTM)
  
    dygraph(actual_48, xlab = "Čas") %>%  
    dySeries("value", label = "Nejsou záznamy") %>%
    dyOptions(colors = RColorBrewer::brewer.pal(1, "Set1"), connectSeparatedPoints = FALSE) %>%
    dyCrosshair(direction = "vertical") %>% 
    dyShading(from = now("GMT") - days(2), to = now("GMT"), color = "#e6e6e6")
  }else{
    actual_48 <- xts::xts(actual_48, order.by = actual_48$DTM)
...
stanice.t <- stanice %>% filter(year %in% input$entry.year) %>% 
                         ...
                         mutate(month = seq(1,12,11/(n()-1))) %>% ungroup()
...
st.long.q <- st.long %>% filter(Q!="median") %>% group_by(Q,month) %>% summarise(Qmin = min(value), Qmax = max(value)) %>% ungroup()
...
st2 <- ggplot()+
  geom_ribbon(data = st.long.q, aes(month, ymin=Qmin, ymax=Qmax, group=Q,fill=Q,
              text=sprintf("mesic: %s<br>kvantil: %s<br>rozpeti: %s - %s", 
                           st.long.q$month, st.long.q$Q, round(st.long.q$Qmin,2), round(st.long.q$Qmax,2))), alpha=0.25)+
  geom_line(data = st.median, aes(month,value, color = factor(Q), group = factor(Q),
                                  text=sprintf("mesic: %s<br>median: %s", st.median$month, round(st.median$value,2))), alpha=0.75)+
  geom_line(data = stanice.t, aes(month, cumsum.val, group = year, colour=factor(year), 
                text=sprintf("DTM: %s<br>hodnota: %s", DTM, round(cumsum.val, 2))))+
  theme_minimal()+
