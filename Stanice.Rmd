...
vuv <- read.table(file.path(.datadir,"libor_d_new\\VUV.txt"), header = TRUE, sep = ";", dec=",", col.names = c("DTM", "Ta", "WV", "WD", "H", "Pyranometr", "E", "P10m", "P1h", "P24h", "Tw"))
...
absolutePanel(class = "panel panel-default", draggable = FALSE,
        ...
        wellPanel(
            ...
            conditionalPanel(condition = "input.entry.var == 'A'",
                             bastynsky <- stanice %>%  filter(ID == "bastynsky") %>% select(-P, -month.P)
                 ),
            conditionalPanel(condition = "input.entry.var == 'B'",
                             bastynsky <- stanice %>%  filter(ID == "bastynsky") %>% select(-TMP, -month.TMP)
                 )
            )
          )
...
renderPlotly({
  ...
  bastynsky.yt <- bastynsky %>% filter(year(DTM) == input$entry.year)
  
  bastynsky.t <- bastynsky %>% select(TMP, DTM, month) %>% group_by(month) %>% 
    mutate(
      TMP10 = quantile(TMP, probs = 0.1), 
      TMP30 = quantile(TMP, probs = 0.3),
      TMP50 = quantile(TMP, probs = 0.5),
      TMP70 = quantile(TMP, probs = 0.7),
      TMP90 = quantile(TMP, probs = 0.9)) %>% select(-DTM, -TMP) %>% ungroup
...
```{r}
bastynsky.g2 <- bastynsky %>% filter(year(DTM) == input$entry.year) %>% select(TMP, month) %>% mutate(cumsum.TMP = cumsum(TMP)) %>% ungroup() %>% arrange(cumsum.TMP)

ggplot(bastynsky.g2, aes(month, cumsum.TMP)) + geom_line()+
    scale_x_continuous(breaks=c(1:12))
```
